...
public class PartyCommandFrontend extends CommandFrontend {
	...
	public void accept(Player p, String[] args) {
		if (Managers.getGroupManager().indexOf(p) != -1) {
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.IINPARTY));
			return;
		}
		if (!Managers.getGroupManager().hasInvite(p)) {
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.INOINVITE));
			return;
		}
		try {
			Managers.getGroupManager().acceptInvite(p);
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.IACCEPT));
	...
	public void create(Player p, String[] args) {
		...
		if (Managers.getGroupManager().hasInvite(p))
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.IDISCARD));
	...
	public void invite(Player p, String[] args) {
		...
		if (Managers.getGroupManager().indexOf(mate) != -1) {
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.ITARGETINPARTY));
			return;
		}
		try {
			Managers.getGroupManager().invite(mate, g);
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.ITARGETINVITED));
		} catch (ManagerException e) {
	...
	public void kick(Player p, String[] args) {
		...
		if (!g.contains(mate)) {
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.ITARGETOUTPARTY));
			return;
		}
		
		try {
			g.remove(mate);
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.IKICKED).replace("%n%", mate.getDisplayName()));
			mate.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.ITARGETKICKED).replace("%n%", p.getDisplayName()));
	...
	public void leave(Player p, String[] args) {
		...
		try {
			...
			g.remove(p);
			p.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.ILEAVE));
		} catch (GroupException e) {
			throw new RuntimeException(e); // toss to CommandFrontend
		}
	...
	public void list(Player p, String[] args) {
		...
		messages.add(ChatUtils.formatHeader(I18NMessage.getLocale().getString(PartyCommandFrontend.ILIST) + " " + members.size() + "/" + g.getCapacity()));
	...
	public void promote(Player p, String[] args) {
		...
		try {
			...
			for (Player member : g.getMembers())
				member.sendMessage(I18NMessage.getLocale().getString(PartyCommandFrontend.IPROMOTE));
		} catch (GroupException e) {
			throw new RuntimeException(e); // throw to CommandFrontend
		}
	}
	...
	public void help(CommandSender sender, String[] args) {
		...
		if (g == null) {
			if (invite)
				...
				messages.add(ChatColor.GRAY + I18NMessage.getLocale().getString(PartyCommandFrontend.INOINVITE));
			...
			if (isLeader) {
				...
				messages.add(ChatColor.GRAY + "[party invite] " + I18NMessage.getLocale().getString(PartyCommandFrontend.INOTLEADER));
				messages.add(ChatColor.GRAY + "[party kick] " + I18NMessage.getLocale().getString(PartyCommandFrontend.INOTLEADER));
			}
			...
			if (isLeader)
				...
				messages.add(ChatColor.GRAY + "[party promote] " + I18NMessage.getLocale().getString(PartyCommandFrontend.INOTLEADER));
		}
